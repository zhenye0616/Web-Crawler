...
def get_cache_server(config, restart):
    ...
    try:
        print(os.path.exists(config.save_file), config.save_file)
        result = init_node.start(config.user_agent, restart or not os.path.exists(config.save_file))
        return result
    except Exception as e:
        # Handle the exception, log the error message, and diagnose the issue
